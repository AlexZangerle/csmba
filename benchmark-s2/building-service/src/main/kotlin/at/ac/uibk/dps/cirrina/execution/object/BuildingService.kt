...
fun main() {
    println("Building Service — Scenario 2 (Comfort Convergence)")
    ...
    // Lighting consumer — only log first turnOn after a positive detection
    app.post("/turnOn") { ctx ->
        if (BenchmarkLog.lightReacted.compareAndSet(false, true)) {
            BenchmarkLog.logEvent("light_consumer", "turnOn")
            println("[LIGHT] on")
        }
        respondEmpty(ctx)
    }
